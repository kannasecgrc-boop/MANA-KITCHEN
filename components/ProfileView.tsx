      ...
      <div className="flex flex-col md:flex-row gap-12 items-start">
        ...
        <div className="w-full md:w-80 bg-white border border-gray-100 rounded-[40px] p-8 shadow-sm">
          <div className="text-center">
            ...
            <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">Active Member</p>
          </div>
          ...
          <div className="mt-10 space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100/50">
              ...
              <p className="text-xs font-bold text-gray-900 leading-relaxed truncate">{user.address || 'Madhira, Khammam'}</p>
            </div>
          </div>
        ...
        <div className="flex-1 space-y-8">
          <div>
            <h3 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Order Workflow</h3>
            <p className="text-gray-500 font-medium">Follow your meal from our kitchen to your door.</p>
          </div>
                ...
                return (
                  <div key={order.id} className="bg-white border border-gray-100 rounded-[40px] p-8 shadow-sm hover:shadow-lg transition-all overflow-hidden border-l-8 border-l-brand">
                    ...
                    <div className="relative pt-6 pb-4 px-4">
                       ...
                       {/* Active Progress */}
                       <div 
                         className="absolute top-1/2 left-4 h-1.5 bg-brand -translate-y-1/2 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(139,0,0,0.3)]"
                    ...
                    <div className="mt-20 p-6 bg-gray-50 rounded-3xl flex items-center gap-5 border border-gray-100">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 leading-none">Kitchen Memo</p>
                          <p className="text-xs font-bold text-gray-700 leading-relaxed">
                            {order.status === 'Pending' && 'Your order is in the queue. We will accept it momentarily!'}
                            {order.status === 'Accepted' && 'Restaurant confirmed! Our chef is looking at your receipt now.'}
                            {order.status === 'Preparing' && 'Tantalizing aromas! Your meal is on the wok right now.'}
                            {order.status === 'Out for Delivery' && 'Handed over to delivery. Our rider is heading your way!'}
                            {order.status === 'Delivered' && 'Great news! We hope you enjoyed the authentic taste.'}
                            {order.status === 'Cancelled' && 'Unfortunately, this order was declined. Contact support for details.'}
                          </p>
                       </div>
                    </div>
